<x-distributor-layout>
    <div class="container p-4 mx-auto">
        <div class="flex items-center justify-between mb-6">
            ...
            <div class="flex space-x-4">
                ...
                <button onclick="openExpiredModal()"
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($discounts as $discount)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                        class="inline-flex px-2 py-1 text-xs font-semibold {{ $discount->type === 'percentage' ? 'text-green-800 bg-green-100' : 'text-blue-800 bg-blue-100' }} rounded-full">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900">
                                        @if ($discount->start_date || $discount->end_date)
                                            {{ optional($discount->start_date)->format('M d, Y h:i A') ?? 'N/A' }} -
                                            {{ optional($discount->end_date)->format('M d, Y h:i A') ?? 'N/A' }}
                                        @else
                                            N/A
                                        @endif
                                    </div>
                                ...
                                <td class="px-6 py-4 text-sm font-medium text-right whitespace-nowrap">
                                    <div class="flex space-x-2">
                                        <button onclick="openEditModal({{ $discount->id }})"
                                            class="text-indigo-600 hover:text-indigo-900">
                                            Edit
                                        </button>
                                        <form action="{{ route('distributors.discounts.toggle', $discount) }}"
                                        ...
                                        <button onclick="confirmDelete({{ $discount->id }}, '{{ $discount->name }}')"
                                            class="text-red-600 hover:text-red-900">
                                            Delete
                                        </button>
                                        <form id="delete-form-{{ $discount->id }}"
                                            action="{{ route('distributors.discounts.destroy', $discount) }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-4 text-center text-gray-500">
                                    No active discounts found. Create your first discount now!
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            <div
                ...
                <div class="relative flex-auto p-6" id="editModalContent">
                    <div class="flex items-center justify-center">
                        <div
                            class="w-12 h-12 border-4 border-t-4 border-gray-200 rounded-full border-t-blue-600 animate-spin">
                        </div>
                    </div>
                </div>
            </div>
    ...
    <!-- Modal backdrop -->
    <div id="modalBackdrop" class="fixed inset-0 z-40 hidden bg-black opacity-25 border-gray-950"></div>


    <!-- Expired Discounts Modal -->
    <div id="expiredDiscountsModal"
        class="fixed inset-0 z-50 flex items-center justify-center hidden overflow-x-hidden overflow-y-auto outline-none focus:outline-none">
        <div class="relative w-full max-w-4xl mx-auto my-6">
            <!-- Modal content -->
            <div
                ...
                <div class="flex items-start justify-between p-5 border-b border-gray-200 rounded-t">
                    ...
                    <button class="float-right text-gray-500 hover:text-gray-800" onclick="closeExpiredModal()">
                        <span class="block w-6 h-6 text-2xl">×</span>
                    </button>
                </div>

                <!-- Modal body -->
                <div class="relative flex-auto p-6" id="expiredModalContent">
                    <div class="flex items-center justify-center">
                        <div
                            class="w-12 h-12 border-4 border-t-4 border-gray-200 rounded-full border-t-blue-600 animate-spin">
                        </div>
    ...
    <!-- Modal backdrop -->
    <div id="expiredModalBackdrop" class="fixed inset-0 z-40 hidden bg-black opacity-25"></div>
        ...
        <script>
            ...
            function closeEditModal() {
                ...
                document.getElementById('editModalContent').innerHTML =
                    '<div class="flex items-center justify-center"><div class="w-12 h-12 border-4 border-t-4 border-gray-200 rounded-full border-t-blue-600 animate-spin"></div></div>';
            }

            function openExpiredModal() {
                document.getElementById('expiredDiscountsModal').classList.remove('hidden');
                document.getElementById('expiredModalBackdrop').classList.remove('hidden');

                // Fetch expired discounts with better error handling
                fetch(`{{ route('distributors.discounts.expired') }}`)
                    .then(response => {
                        if (!response.ok) {
                    ...
                    .then(html => {
                        document.getElementById('expiredModalContent').innerHTML = html;
                    })
                    .catch(error => {
                        console.error('Error details:', error);
                        document.getElementById('expiredModalContent').innerHTML =
                            `<div class="p-4 text-red-600">
                    <p>Failed to load expired discounts.</p>
                    <p>Error: ${error.message}</p>
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Error',
                            text: 'Failed to load expired discounts. Please try again.'
                        });
                    });
            }

            function closeExpiredModal() {
                document.getElementById('expiredDiscountsModal').classList.add('hidden');
                document.getElementById('expiredModalBackdrop').classList.add('hidden');
                document.getElementById('expiredModalContent').innerHTML =
                    '<div class="flex items-center justify-center"><div class="w-12 h-12 border-4 border-t-4 border-gray-200 rounded-full border-t-blue-600 animate-spin"></div></div>';
            }
                            ...
                            .then(response => {
                                if (!response.ok) {
                                    return response.text().then(err => {
                                        console.error('Error response:', err); // Log the raw response
                                        throw new Error(
                                            'Failed to update discount. Please check the form and try again.'
                                        );
                                    });
                                }
