<x-app-layout>
    ...
    <div class="py-6 sm:py-12">
        <div class="container px-4 mx-auto">
            <!-- Search Form - Made more compact on mobile -->
            <form action="{{ route('retailers.search') }}" method="GET" class="max-w-2xl mx-auto mb-6 sm:mb-8">
                <div class="flex gap-0">
                    <div class="relative w-full">
                        <input type="search" id="search-dropdown" name="query"
                            ...
                            placeholder="Search products or distributors..." required
                            value="{{ request('query') }}" />
                        <button type="submit"
            ...
            <div class="mb-6 sm:mb-8">
                <h2 class="mb-4 text-lg sm:text-xl font-semibold">Search Results for "{{ request('query') }}"</h2>

                <!-- Distributors Results -->
                @if ($distributors->isNotEmpty())
                    <div class="mb-6 sm:mb-8">
                        <h3 class="mb-3 sm:mb-4 text-base sm:text-lg font-medium">Distributors</h3>
                        <div class="grid grid-cols-1 gap-3 sm:gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                            @foreach ($distributors as $distributor)
                                <a href="{{ route('retailers.distributor-page', $distributor->id) }}"
                                    class="flex flex-col items-center p-4 sm:p-6 transition-all duration-300 bg-white border border-gray-100 shadow-md hover:shadow-xl rounded-xl group">
                                    <img class="w-16 h-16 sm:w-24 sm:h-24 mb-3 sm:mb-4 transition-transform duration-300 rounded-full shadow-md group-hover:scale-110"
                                    ...
                                    <h3 class="text-sm sm:text-lg font-bold text-gray-800 text-center">{{ $distributor->company_name }}</h3>
                                </a>
                            @endforeach
                        </div>
                    </div>
                ...
                @if ($products->isNotEmpty())
                    <div>
                        <h3 class="mb-3 sm:mb-4 text-base sm:text-lg font-medium">Products</h3>
                ...
                @if ($distributors->isEmpty() && $products->isEmpty())
                    <div class="p-4 sm:p-8 text-center text-gray-500 rounded-lg bg-gray-50">
