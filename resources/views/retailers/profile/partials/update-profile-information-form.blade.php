<section>
    ...
    <form method="post" action="{{ route('retailers.profile.update') }}" class="mt-6 space-y-6">
        @csrf
        @method('patch')
        ...
        <div>
            ...
            <x-text-input id="middle_name" name="middle_name" type="text" class="block w-full mt-1" :value="old('middle_name', $user->middle_name)"
                required autofocus pattern="^[a-zA-Z\s\-']+$"
                oninput="this.value = this.value.replace(/[^a-zA-Z\s\-']/g, '')" autocomplete="last_name" />
