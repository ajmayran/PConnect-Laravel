<x-app-layout>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 sm:gap-6">
            ...
            <div class="flex justify-center sm:justify-start gap-3 sm:gap-4">
                ...
                <a href="{{ route('retailers.messages.index', ['distributor' => $distributor->user_id]) }}"
                    class="flex items-center px-3 py-2 sm:px-4 sm:py-2 text-sm sm:text-base text-white bg-green-500 rounded-lg hover:bg-green-600 active:bg-green-700 transition-colors duration-200 touch-manipulation">
                    <svg class="w-4 h-4 sm:w-5 sm:h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <!-- Categories -->
    <div class="container px-2 sm:px-4 mx-auto mb-2 sm:mb-4 bg-white rounded-lg shadow-lg">
            ...
            @forelse($products as $product)
                <a href="{{ route('retailers.products.show', $product->id) }}"
                    ...
                    <div class="flex items-center justify-between">
                        <span
                            class="text-sm sm:text-lg font-bold text-green-600">₱{{ number_format($product->price, 2) }}</span>
                        <span
                            class="px-3 py-1 sm:px-4 sm:py-2 text-xs sm:text-sm font-medium text-white bg-green-500 
