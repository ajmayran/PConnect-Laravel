...
<div x-data="{ open: false }" x-on:open-modal-review.window="open = true" x-on:close-modal-review.window="open = false">
    ...
    <button @click="open = true" class="flex items-center px-4 py-2 text-white bg-green-500 rounded-lg hover:bg-green-600">
                    ...
                    <form action="{{-- route('retailers.reviews.store') --}}" method="POST" class="mt-4">
                        ...
                        <div class="flex items-center mb-4" x-data="{ rating: 0 }">
                            <div class="flex items-center space-x-1">
                                @for ($i = 1; $i <= 5; $i++)
                                    <button type="button"
                                        ...
                                        <svg class="w-6 h-6 transition-colors duration-200" 
                                            :class="{'text-yellow-300': rating >= {{ $i }}, 'text-gray-300': rating < {{ $i }}}"
                                ...
                                <span class="ml-2 text-sm text-gray-600" x-text="rating ? `${rating} Stars` : 'Select Rating'"></span>
                            </div>
                        </div>

                        <!-- Review Text -->
                        <div class="mt-4">
                            <textarea name="review" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500" placeholder="Write your review..."></textarea>
                        </div>
