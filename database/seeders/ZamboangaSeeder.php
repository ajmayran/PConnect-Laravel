...
class ZamboangaSeeder extends Seeder
{
    public function run()
    {
        ...
        try {
            ...
            if (!$region) {
                // Get the structure of the regions table to check required fields
                ...
                // Add region_id if it's required (using the same value as code)
                if (in_array('region_id', $regionColumns)) {
                    $regionData['region_id'] = $regionCode;
                }

                DB::table('regions')->insert($regionData);
                $region = Region::where('code', $regionCode)->first();
                $this->command->info('Region: ' . $region->name . ' created');
            } else {
                $this->command->info('Region: ' . $region->name . ' already exists');
            }
            ...
            $barangays = [
                ['code' => '09317089', 'name' => 'Arena Blanco'],
                ...
                ['code' => '09317095', 'name' => 'Pasobolong'],
                ['code' => '09317096', 'name' => 'Patalon'],
